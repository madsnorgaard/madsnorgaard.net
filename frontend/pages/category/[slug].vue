...
const { data } = await useFetch<any>(`/api/wp/categories/${route.params.slug}`)
...
const termName = data.value?.term?.name || 'Category'
const firstImage = data.value?.posts?.[0]?.featuredImage?.src || ''

useHead({
  title: `${termName} | Category`,
  meta: [
    { name: 'description',         content: data.value?.term?.description || `Posts in the ${termName} category` },
    { property: 'og:title',        content: `${termName} | Category` },
    { property: 'og:description',  content: data.value?.term?.description || `Posts in the ${termName} category` },
    { property: 'og:image',        content: firstImage },
    { property: 'og:url',          content: `https://madsnorgaard.net/category/${data.value?.term?.slug}` },
    { property: 'og:type',         content: 'website' },
    { name: 'twitter:card',        content: firstImage ? 'summary_large_image' : 'summary' },
    { name: 'twitter:title',       content: `${termName} | Mads Nørgaard` },
    { name: 'twitter:description', content: data.value?.term?.description || `${termName} category` },
