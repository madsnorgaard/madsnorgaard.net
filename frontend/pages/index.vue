<template>
  <div>
    ...
    <!-- ─── Photo gallery ───────────────────────────────────────────── -->
    <div v-if="photos?.length" class="container" style="margin: 3rem auto;">
...
const [{ data: about }, { data: posts }, { data: projects }, { data: status }, { data: photos }] = await Promise.all([
  ...
  useFetch<any[]>('/api/wp/photos', { query: { per_page: 30 } }).catch(() => ({ data: ref([]) })),
])
