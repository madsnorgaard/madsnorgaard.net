// GET /api/wp/tags/:slug
// Returns a WP tag term and its posts (standard WP posts, not custom post types).
// Mirrors the WordPress /tag/{slug}/ archive page.
...
export default defineEventHandler(async (event) => {
  ...
  // Resolve tag term by slug
  const terms = await wpFetch<any[]>(
    `${base}/wp-json/wp/v2/tags?slug=${encodeURIComponent(slug)}`
  )
  const term = terms?.[0]
  if (!term) {
  ...
  const { data: posts, total, totalPages } = await wpFetchWithHeaders<any[]>(
    `${base}/wp-json/wp/v2/posts?tags=${term.id}&per_page=${perPage}&page=${page}&_embed=wp:featuredmedia,wp:term&status=publish&orderby=date&order=desc`
  )
